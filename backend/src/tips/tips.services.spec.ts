...
describe('TipsService', () => {
  ...
  describe('create', () => {
    ...
    it('should create a tip successfully', async () => {
      ...
      mockUsersService.findOne.mockResolvedValue({ id: createTipDto.artistId, isArtist: true });
      mockStellarService.getTransactionDetails.mockResolvedValue({
        successful: true,
        operations: [{ type: 'payment', to: 'artist_wallet_addr', amount: '10.5' }],
      });
    ...
    it('should throw ConflictException if stellar transaction hash already exists', async () => {
      ...
      await expect(service.create(userId, createTipDto)).rejects.toThrow(ConflictException);
    ...
    it('should throw BadRequestException if user tries to tip themselves', async () => {
      ...
      await expect(service.create(userId, selfTipDto)).rejects.toThrow(BadRequestException);
      expect(mockTipRepository.save).not.toHaveBeenCalled();
    });
  });
